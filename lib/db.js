...
export async function loadState(sql = getSql()) {
  ...
  for (const t of tableRows) {
    tables[t.id] = { capacity: Number(t.capacity), guests: [] };
  }
...
function normalizePayload(payload) {
  ...
  for (const [tableId, t] of Object.entries(tables)) {
    ...
    tableMap.set(String(tableId), { capacity, guests: seated });
  }
...
export async function saveState(payload, sql = getSql()) {
  ...
  // Keep existing seatings if client sent empty tables by mistake
  if (tableMap.size === 0) {
  ...
  // Upsert all guests in one query
  if (guestIds.length) {
    await sql`
      INSERT INTO guests (id, name)
  ...
  if (tableIds.length) {
    await sql`
      INSERT INTO tables (id, capacity)
      SELECT * FROM UNNEST(${tableIds}::text[], ${capacities}::int[])
      ON CONFLICT (id) DO UPDATE SET capacity = EXCLUDED.capacity
    `;
  }

  // Rebuild seatings from payload
