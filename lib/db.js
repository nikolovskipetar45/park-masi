...
function normalizeGuestOwner(owner) {
  const id = String(owner || "").trim();
  return VALID_OWNERS.has(id) ? id : null;
}
...
export async function loadState(sql = getSql()) {
  ...
  const guestRows = await sql`
    SELECT id, name, checked_in, owner FROM guests ORDER BY id
  `;
  const tableRows = await sql`SELECT id, capacity, owner FROM tables`;
  const seatRows = await sql`
    SELECT s.table_id, s.guest_id, s.position, g.name, g.checked_in, g.owner
    FROM seatings s
  ...
  for (const s of seatRows) {
    ...
    tables[s.table_id].guests.push({
      ...
      owner: normalizeGuestOwner(s.owner),
    });
  }
  ...
  return {
    guests: guestRows.map((g) => ({
      ...
      owner: normalizeGuestOwner(g.owner),
    })),
    tables,
  };
}
...
function normalizePayload(payload) {
  ...
  for (const [tableId, t] of Object.entries(tables)) {
    ...
    list.forEach((g, position) => {
      ...
      const prev = guestMap.get(gid);
      const name = String(g.name || "").trim() || prev?.name || `guest-${gid}`;
      const guestOwner = normalizeGuestOwner(g.owner) || prev?.owner || null;
      guestMap.set(gid, { name, owner: guestOwner });
...
export async function saveState(payload, sql = getSql()) {
  ...
  const guestNames = guestIds.map((id) => guestMap.get(id).name);
  const guestOwners = guestIds.map((id) => guestMap.get(id).owner);

  if (guestIds.length) {
    // Preserve checked_in on existing guests; new guests default to false
    await sql`
      INSERT INTO guests (id, name, checked_in, owner)
      SELECT id, name, false, owner
      FROM UNNEST(
        ${guestIds}::int[],
        ${guestNames}::text[],
        ${guestOwners}::text[]
...
export async function listCheckins(sql = getSql()) {
  ...
  return rows.map((r) => ({
    ...
    owner: normalizeGuestOwner(r.table_owner) || normalizeGuestOwner(r.guest_owner),
  }));
}
...
export async function setCheckedIn(guestId, checkedIn, sql = getSql()) {
  ...
  const seat = await sql`
    SELECT s.table_id, t.owner AS table_owner, g.owner AS guest_owner
    FROM guests g
    LEFT JOIN seatings s ON s.guest_id = g.id
    LEFT JOIN tables t ON t.id = s.table_id
    WHERE g.id = ${id}
  ...
  return {
    ...
    owner:
      normalizeGuestOwner(seat[0]?.table_owner) ||
      normalizeGuestOwner(seat[0]?.guest_owner),
  };
}
...
function normalizeGuestIds(guestIds, { allowEmpty = false } = {}) {
  const ids = [...new Set((guestIds || []).map((id) => Number(id)).filter((id) => Number.isFinite(id)))];
  if (!ids.length && !allowEmpty) throw new Error("Select at least one guest");
  return ids;
}

function mapGiftGuests(guests) {
  return (guests || []).map((g) => ({
    id: Number(g.id),
    name: g.name,
    owner: normalizeGuestOwner(g.owner),
      ...
      COALESCE(
        json_agg(
          json_build_object(
            'id', g.id,
            'name', g.name,
            'owner', COALESCE(t.owner, g.owner)
          )
          ORDER BY g.name ASC, g.id ASC
...
export async function listGifts(sql = getSql()) {
  ...
  const guestRows = await sql`
    SELECT
      g.id,
      g.name,
      COALESCE(t.owner, g.owner) AS owner
    FROM guests g
    LEFT JOIN seatings s ON s.guest_id = g.id
    LEFT JOIN tables t ON t.id = s.table_id
    ORDER BY g.name ASC, g.id ASC
...
export async function createGift(guestIds, amount, sql = getSql()) {
  await ensureSchema(sql);
  const ids = normalizeGuestIds(guestIds, { allowEmpty: true });
